package org.port.pokemon.api.services;

import lombok.RequiredArgsConstructor;
import org.port.pokemon.api.entities.ApiPokemon;
import org.port.pokemon.api.entities.ApiResponse;
import org.port.pokemon.api.entities.UrlItem;
...
public class ApiUpdateService {
    ...
    public void update(int page) {
        ...
        for (UrlItem item : items) {
            ...
            // 포켓몬 분류
            String genus = data2.getGenera().stream().filter(d->d.getLanguage()
                    .getName().equals("ko")).map(d->d.getGenus()).collect(Collectors.joining());
